...
import jwt, json, rq, redis, base64, os
...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    # Add notification
    def update_notification(self, name, data):
    ...
    def launch_task(self, name, description, *args, **kwargs):
        # Create a task and enqueue to the redis queue
        rq_job = current_app.task_queue.enqueue('app.tasks.'+name, self.id, *args, **kwargs)
        # Save the equivalent task to database
    ...
    def to_dict(self, include_email=False):
        data = {
            ...
            'idol_count': self.idols.count(),
            '_links': {
                'self': url_for('api.get_user', id=self.id),
                'followers': url_for('api.get_followers', id=self.id),
                'idols': url_for('api.get_idols', id=self.id),
                'avatar': self.avatar(32)
            }
    ...
    def revoke_token(self):
        # Make the current token stabled
